...
//
// Copied verbatim from the tebako-rs workspace
// (crates/tebako-signer/src/bz_internal_shim.c, where the shim was
// proven against the rnp-rs 0.1.7 static link). Here it is archived into
// the feedstock's own libbz2.a so the hole is fixed at the source, once,
// for every consumer of the toolkit.
